...
```package-install
npm install @one-stack/use-audio-analyser
```

## Quick Start

```tsx twoslash include import
import { useAudioAnalyser } from '@one-stack/use-audio-analyser'
```
...
```tsx twoslash
import { useUserMedia } from '@one-stack/use-user-media'
import { useAudioAnalyser } from '@one-stack/use-audio-analyser'
...
Although data is always an array of numbers, changing the method parameter will
change the size and the values of the array as follows:

| AnalyserMethod           | data type                                | data size     |
| ------------------------ | ---------------------------------------- | ------------- |
| `getByteFrequencyData`   | `number[]` created from a `Uint8Array`   | `fftSize / 2` |
| `getFloatFrequencyData`  | `number[]` created from a `Float32Array` | `fftSize / 2` |
| `getByteTimeDomainData`  | `number[]` created from a `Uint8Array`   | `fftSize`     |
| `getFloatTimeDomainData` | `number[]` created from a `Float32Array` | `fftSize`     |
