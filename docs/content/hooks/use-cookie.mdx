...
## Read, Write & Clear cookies outside of React

You can read and write cookies outside of React by using the `clientCookies` object.

```ts twoslash include clientcookies
import { z } from 'zod'
import { defineCookies } from '@one-stack/use-cookie'
// ---cut---
export const { CookieProvider, useCookie, clientCookies } = defineCookies({
  accessToken: {
...
Using the `clientCookies` to write or clear cookies will still trigger a **React re-render**.

```tsx twoslash
// @noErrors
// @include: clientcookies
// ---cut---
import { clientCookies } from './cookies'

// Write a cookie
clientCookies.set('accessToken', '123')

// Read a cookie
const accessToken = clientCookies.get('accessToken')

// Clear specific cookies
clientCookies.clear(['accessToken'])
...
You can read cookies on the server by using the `serverCookies` object.

```ts twoslash include servercookies
import { z } from 'zod'
import { defineCookies } from '@one-stack/use-cookie'
// ---cut---
export const { CookieProvider, useCookie, serverCookies } = defineCookies({
  accessToken: {
    validate: z.string().optional(),
    expires: 7, // days from now
  },
...
The `serverCookies` has only one `get` method, which expect the headers and the cookie name. \
It ensures the cookie is properly **decoded**, **deserialized** and **validated**.

```tsx twoslash
// @noErrors
// @include: servercookies
// ---cut---
import { headers } from 'next/headers'
import { serverCookies } from './cookies'

const accessToken = serverCookies.get(await headers(), 'accessToken')
```
...
```ts
const clientCookies = { set; clear; get }
```
