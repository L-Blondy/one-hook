...
test('<CookieProvider> requires the `ssrCookies`', () => {
  const { CookieProvider } = defineCookies({}, {})
  // @ts-expect-error missing ssrCookies
  const _ = <CookieProvider>children</CookieProvider>
  // ok
  render(<CookieProvider ssrCookies={{}}>children</CookieProvider>)
})
...
test('type inferrence', () => {
  ...
  function CookieWrapper(props: { children: React.ReactNode }) {
    return <CookieProvider ssrCookies={{}}>{props.children}</CookieProvider>
  }
})

test('ssrCookies should be used for the initial State, decoded, parsed and validated', () => {
  const { CookieProvider, useCookie } = defineCookies({
    ch1: { validate: (value) => Number(value) },
    ch2: { validate: (value) => String(value) },
...
test('Should be just like useState, but shared', () => {
  ...
  function CookieWrapper(props: { children: React.ReactNode }) {
    return <CookieProvider ssrCookies={{}} {...props} />
  }
  const { result } = renderHook(
    () => {
      const [state1, setState1] = useCookie('ch1')
...
test('hooks should sync when `ssrCookies` change', () => {
  const { rerender } = render(<TestApp ssrCookies={{ ch1: '20' }} />)
  screen.getByText('Consumer1 20')
  rerender(<TestApp ssrCookies={{ ch1: '30' }} />)
...
function TestApp(props: {
  ...
  return (
    <AppCookiesProvider ssrCookies={props.ssrCookies ?? {}}>
      <Consumer1 />
