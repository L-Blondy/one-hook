...
test("validate schema type inferrence", () => {
  ...
  const l2_2 = cookieService_2.get("k2", {
...
/**
 * Runtime can't be tested as to cookies are not available outside of the browser
 */
...
test("In/Out objects should be equal", async () => {
  ...
  expect(await cookieService.get("cookie1", { validate: (x) => x })).toEqual({
    a: new Date("2000-01-01").toISOString(),
    b: null,
    c: false,
    d: 0,
